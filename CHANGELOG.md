...
- [IMPROVEMENT] The retention purge now deletes expired logs and requests in batches of 5,000 rows, oldest first, instead of one `DELETE` over the whole backlog. A site that built up a large table before retention was always on (hundreds of thousands of rows) no longer risks a long table lock or a timed-out cron run on its first purge. Each run stops after about 20 seconds, logs how far it got, and schedules a follow-up run five minutes later until the backlog is gone. Tune it with the `swpl_purge_batch_size` and `swpl_purge_time_budget` filters; the `swpl_purged` action receives each run's totals.
- [CHANGED] The purge cutoff is now computed in GMT, the same clock the plugin writes `date_created` with, instead of the database server's `NOW()`. On a database server set to a non-UTC time zone, rows were kept a few hours too long or too short.
