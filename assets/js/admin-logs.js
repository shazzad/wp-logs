...
(function ($) {
	...
	$(document).ready(function () {
		...
		$(document.body).on('click', 'table.swpl-logs .row-actions .view a', function () {
			var id = $(this).closest('tr').attr('id').split('swpl-log-')[1];

			$(document.body).trigger('swpl-modal-init');
			$(document.body).trigger('swpl-modal-body', $('#swpl-log-modal-' + id).html());
			$(document.body).trigger('swpl-modal-show');

			return false;
		});

		$(document.body).on('click', 'table.swpl-logs .row-actions .delete a', function () {
			...
			$.post(ajaxurl, data)
				.done(function (r) {
					if (r.success) {
						...
						var oldDisplayingNum = $('.tablenav-pages .displaying-num').html().replace(' items', '');
						var newDisplayingNum = parseInt(oldDisplayingNum, 10) - 1;
						$('.tablenav-pages .displaying-num').html(newDisplayingNum + ' items')
